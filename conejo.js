...
var jugar = {
	...
	y=juego.input.pointer1.y;
		if(flechaderecha.isDown ||(juego.input.pointer1.isDown && x>75 && (Math.abs(y-664)>x-75))){
			this.caminader();
			personaje.animations.play("righta");
		}else if(flechaizquierda.isDown ||(juego.input.pointer1.isDown && x<75 && (Math.abs(y-664)>x-75))){
			this.caminaizq();
			personaje.animations.play("lefta");
		}else if(flechaarriba.isDown){
			this.arriba();
			personaje.animations.play("upa");	
		}else if(flechaabajo.isDown){
			this.abajo();
